...
export function getDepsInfo(cwd: string): DepsInfo {
  const normalizedCwd = normalizePath(cwd);
...
export function getFileStats(cwd: string, maxDepth: number = 2): { totalFiles: number; totalDirs: number; extensions: Map<string, number> } {
  const stats = {
    totalFiles: 0,
    totalDirs: 0,
...
export function getTopExtensions(stats: { extensions: Map<string, number> }, max: number): Map<string, number> {
