...
import { setLanguage, getLabels, t } from './i18n/index.js';
...
function renderGitLine(gitStatus: RenderContext['gitStatus'], config: RenderContext['config']): string {
  ...
  if (gitStatus.staged > 0 || gitStatus.unstaged > 0 || gitStatus.untracked > 0) {
    ...
    if (gitStatus.staged > 0) changeParts.push(`+${gitStatus.staged}`);
    if (gitStatus.unstaged > 0) changeParts.push(`~${gitStatus.unstaged}`);
    if (gitStatus.untracked > 0) changeParts.push(`?${gitStatus.untracked}`);
...
function renderGitHubLine(githubStatus: RenderContext['githubStatus'], config: RenderContext['config']): string {
  const labels = getLabels().labels;
...
function renderDepsLine(depsInfo: RenderContext['depsInfo'], config: RenderContext['config']): string {
  const labels = getLabels().labels;
...
function renderTipsLine(config: RenderContext['config']): string {
  const labels = getLabels().labels;
